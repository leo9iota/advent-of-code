...
async function computeCalibrationValues(filePath: string) {
    try {
        ...
        for (const line of lines) { // Loop over the separate strings
            ...
            for (let i = 0; i < line.length; i++) { // Loop over the string by its length
                const number: number = parseInt(line[i]); // Parses the single character to an integer

                if (!isNaN(number)) { // Check if the parsed integer is a number
                    if (firstDigit === null) { // Check if the first digit variable is still unassigned
            ...
            if (firstDigit !== null && lastDigit !== null) { // Check if first and last digit have been assigned
            ...
            } else if (firstDigit !== null) { // Check if theres only one digit
